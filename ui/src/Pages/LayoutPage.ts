...
import donatecode from "./donatecode.html";
...
export class LayoutPage extends HeaderPage {
  constructor(){
    ...
    this.data = Objects.copy(dataClean);

    /** @type {import("leet-mvc/typings/index").FieldTemplate} */
    this.controlPropertiesForm = { type:"form", class:"box", displayRule:"true_if_not:controllType,null", items:[
      ...
      { type:"select", name:"container", title:"Element Screen", placeholder:"", validateRule:"required", class:"", items: [
        { value:"AS3", title: "Main Screen"},
        { value:"AS3jog", title: "Jog Screen"},
      ]},
      ...
      { type:"number", title:"Parent Tab Layer Number", name:"parentN", displayRule:`true_if:controllType,${TabLayer.name}`, validateRule:"required|numeric|min:0" },
      { type:"number", title:"Layer Number", name:"layerN", displayRule:`true_if_not:controllType,${PictureNode.name}`, validateRule:"required|numeric|min:1"},
      { type:"form", class:"row", displayRule:`true_if_not:controllType,${PictureNode.name}`, items:[
        ...
        { type:"number", name:"h", title:"Height", placeholder:"", displayRule:`true_if:controllType,${ButtonNode.name},${BackgroundNode.name},${UCCAMNode.name},${ListNode.name},${LedNode.name},${ToolpathNode.name},${TabLayer.name},${FillNode.name}`, validateRule:"required|numeric|min:0", class:"col-6", unit:"px"},
      ]},
      { type:"form", class:"row", items:[
        { type:"checkbox", name:"toggle", title:"Toggle", attributes:{title:"Element will have a secondary toggle state marked by the secondary image"}, displayRule:"true_if:controllType,"+ButtonNode.name+"", value: false, class:"col-6"},
      ...
      { type:"form", class:"", displayRule:`true_if:controllType,${ButtonNode.name},${BackgroundNode.name},${LedNode.name},${TabLayer.name}`, items:[
        { type:"picture-select", name:"picN", title:"Picture Number", placeholder:"", validateRule:"required|numeric|min:0"},
      ...
      { type:"form", class:"row", displayRule:`true_if:controllType,${FieldNode.name},${ListNode.name},${CodeviewNode.name},${LabelNode.name}`, items:[
      ...
      { type:"select", name:"align", title:"Text Align", placeholder:"", displayRule:"true_if:controllType,"+FieldNode.name+","+ListNode.name+","+LabelNode.name+"", validateRule:"required", items:[
    ...
    this.form.onChange = (ev)=>{
      ...
      if (!this.form.validator.isValid) {
        Alert("Some of the Element Properties are not valid.\nPlease fix issues before continuing!", null, "Element will not be updated!");
        return;
      }
      if (ev) {
    ...
    /*this.userSignin = new UserSignIn(config.API_URL)

    this.userSignin.userSigninStatusChanged.onChange((status)=>{
      this.isSignedIn = !!status;
      this.userInfo = status
      this.menuButton = !!status;
    })

    this.userSignin.checkLoginStatus();*/
  ...
  onMenuButtonClicked(){
    let d = this.Nav.push(ActionSheetPage)
    d.items = [
      { icon: "fa fa-user", title: "Log Out", name:"logout" }
    ];

    if (this.userInfo.role_id == UserRole.admin) {
      d.items.push({
        icon: "fa fa-users", title: "User Report", name:"edit-users"
      });
    }

    d.onItemClicked = (item)=>{
      if (item.name=="logout") {
        this.parser = new Parser();
        this.clearAll()
        this.userSignin.setToken(null);
      }

      if (item.name=="edit-users") {
        this.userSignin.showEditUsers();
      }
    }
  }
  ...
  async reRender(){
    //this.form.errors = {};
    this.parseErrors = [];
    this.pictures = Objects.filter(this.parser.getNodes(), n=> (n instanceof PictureNode && n.region == this.data.controller )) as PictureNode[];
    
    this.pictures.sort(function(a,b){
      return a.controllN - b.controllN
    })

    let currentNodes = Objects.filter(this.parser.getNodes(), node => node.region==this.data.controller && this.data.controller) as any[];
  ...
  clearAll(){
    ...
    Objects.overwrite(this.data, dataClean);
  ...
  async renderMainArea(){
    ...
    //clear el.nodes from
    let currentNodes = Objects.filter(this.parser.getNodes(), node => node.region==this.data.controller && node.container=="AS3" ) as any[];
    Objects.forEach(currentNodes, node=>node.el = null);
  ...
  onInit(){
    ...
    DOM(document).on("keydown",(event: KeyboardEvent)=>{
      if(event.which==17)
    ...
    DOM(document).on("keyup",(event: KeyboardEvent)=>{
      this.ctrlPressed = false;
      this.shiftPressed = false;
      this.altPressed = false;
    })
    ...
    DOM(this.workArea).on("mousedown",(event: MouseEvent)=>{
      const target = event.target as HTMLElement;
      if (!this.shiftPressed && /*target.classList.contains("UC-control") &&*/ !target.classList.contains("selected")){
        //start selecting
        ...
        var x2=0, y2=0;
        var parentElement = (DOM(target).closest("#mainarea").first() || DOM(target).closest("#jogarea").first()) || target
        
        DOM(this.workArea).on("mousemove",(event: MouseEvent)=>{
      ...
      if ((event.target as HTMLElement).classList?.contains("UC-control") && (event.target as HTMLElement).classList?.contains("selected")){
    ...
    DOM(window).on("beforeunload",  (e: BeforeUnloadEvent)=>{
      if (this.pendingSave) {
        var confirmationMessage = 'You have unsaved changes!'
                                + '\nIf you leave before saving, your changes will be lost.';
    
        (e as any).returnValue = confirmationMessage; //Gecko + IE
  ...
  onFitYClicked(){
    ...
    Objects.forEach(this.selectedNodes, (node, i)=>{
      node.y = node1.y + round(Number(dY) * Number(i), 0);
      node.el.style.top =  node.y + "px";
    })
    this.saveHistorySnapshot();
  }
  ...
  onDeleteControlsClicked(){
    ...
    if (this.selectedNodes.find(el=>el instanceof TabLayer)) {
      p.addCheck("recursive", "Recursively delete contents of TabNodes", null, false);
    }
  ...
  onCreateControlClicked(){
    ...
    p.addActionButton("Add", ()=>{
      ...
      switch(p.data.controllType){
        case TabLayer.name: node = new TabLayer; break;
      ...
      Objects.forEach(p.data, (value, key)=>{
        if (value != null)
          node[key] = value;
      })
  ...
  onEditScreenProperttiesClicked(){
   ...
   var jogCollapsed = this.parser.getNodes().find(el=>el instanceof SetJogPanelTabSizeNode && el.region == this.data.controller);
    ...
    p.controls = [
      ...
      { type:"text", name:"tabSelectOrder", title:"Tab Layer Select Order (comma-separated numbers)", placeholder:"", validateRule:"required"},
    ];
    p.render();
    ...
    p.addActionButton("Ok", ()=>{
      ...
      if (!String(p.data.tabSelectOrder).match(/^[0-9]+(\,[0-9]+)*$/)) {
        p.errors["tabSelectOrder"] = "The Tab Layer Select Order list is in incorrect format."
        return false;
      }
      /*check if layerselect is valid!*/ 
      let layernaumbers = String(p.data.tabSelectOrder).split(",").map(el=>Number(el));
      let repeating = layernaumbers.filter((item, index) => layernaumbers.indexOf(item) !== index)
      if (repeating.length>0) {
        p.errors["tabSelectOrder"] = "The Layer Numbers '"+repeating.join(",")+"' appear more than once in the Tab Layer Select Order list."
        return false;
      }
      let tabNodes = this.parser.getNodesWhere(this.data.controller, ScreenName.AS3, TabLayer);
      let noLayers = layernaumbers.filter(el=>!tabNodes.find(el1=>el == el1.layerN));
      if (noLayers.length > 0) {
        p.errors["tabSelectOrder"] = "The Layer Number '"+noLayers.join(",")+"' does not exist in this Screen"
        return false;
      }

      //remove all old selects:
      let oldSelects = this.parser.getNodesWhere(this.data.controller, ScreenName.AS3, SelectLayerNode);
      NodeOperations.removeNodes(oldSelects,this.parser, false);
      //recreate selects according to the new list:
      layernaumbers.forEach(ln=>{
        let sln = new SelectLayerNode();
        sln.region = this.data.controller
        sln.container = ScreenName.AS3
        sln.layerN = ln;
        this.parser.insertNewNode(sln)
      })
      //end
    ...
    p.data = Objects.overwrite(p.data, {
      ...
      tabSelectOrder: this.parser.getNodesWhere(this.data.controller, ScreenName.AS3, SelectLayerNode).map(el=>el.layerN).join(',')
  ...
  onApplyNodesStyleOnOtherControllersClicked(){
    ...
    let controllers = this.parser.getRegions();
    controllers.forEach(controller=>{
        if (controller != this.data.controller){
          p.addCheck(controller, controller, null, false)
        }
    })
  ...
  onCopySelectedControlsClicked(){
    var p = Dialog("Copy Elements");
    p.addLabel(null, "This dialog allows you to copy the selected elements into another controller.")

    p.addLabel(null,"Select the Controllers where you want to copy the " + this.selectedNodes.length + " selected elements:")

    let controllers = this.parser.getRegions();
    controllers.forEach(controller=>{
        if (controller != this.data.controller && controller != "SCREENPROPERTIES"){
          p.addCheck(controller, controller, null, false)
        }
    })

    if (this.selectedNodes.find(el=>el instanceof TabLayer)) {
      p.addCheck("recursive", "Recursively copy contents of TabNodes", null, false);
    }


    p.addActionButton("Cancel", null);

    let orig_regions = this.parser.getRegions();

    p.addActionButton("Copy", ()=>{

      let regions = orig_regions.filter(el=>{
        return p.data[el]
      })
      ...
      try {
        regions.forEach(region=>{
          NodeOperations.copyNodeToOtherRegions(this.selectedNodes, region, this.parser, p.data.recursive)
        })
      } catch (err){
        Alert(err.message, null, "Something Wrong!")
      }
  ...
  get isSelectedElementPictureGenSupported(){
    return this.selectedNodes && this.selectedNodes.length == 1 &&  (this.selectedNodes[0] instanceof ButtonNode || this.selectedNodes[0] instanceof TabLayer || this.selectedNodes[0] instanceof BackgroundNode || this.selectedNodes[0] instanceof LedNode )
  }
  ...
  async onSignInRegisterClicked(){
    await this.userSignin.showLoginDialog();
  }
  ...
  onLoadSampleScreensetClicked(ss){
    ...
    FetchFile(file).then(async (text)=>{
      this.history = [];
      await this.parse(text, false);
    })
  ...
  async onSaveScreensetClicked(){
    ...
    let loader = Loader().show("Saving Screenset...")
    setTimeout(async() => {
      try {
        ...
        Alert(error.message, null, "Save Error");
      }
      loader.hide();      
    }, 100);
  ...
  onWorkAreaClicked(ev){
    ...
    if (!this.ctrlPressed) {
      ...
      if (target.classList.contains("TabLayer")) {
   ...
   */
  getNodeTabNode(node){
    if (node instanceof TabLayer) {
      return node;
    }

    let ret = Objects.find(this.parser.getNodes(), n=> n.region == this.data.controller && n instanceof TabLayer && n.layerN == node.layerN )
    return ret;
  }
  ...
  showTabLayer(layerN){
    /** @type {TabLayer[]} */
    // @ts-ignore
    var tabs = Objects.filter(this.parser.getNodes(), node=> node.region == this.data.controller && node instanceof TabLayer )
   ...
   * @param {CNode} node 
   */
  selectNode(node, multiple = false, selectControlTree = true){
    ...
    if (node instanceof PictureNode) {
      ...
      if (this.selectedNodes.find(el=>el == node)) {
        this.selectedNodes = this.selectedNodes.filter(el=>el != node);
      } else {
        if (multiple) {
      ...
      this.form.validator.clearErrors();
      this.controlTree.setSelectedNodes(this.selectedNodes);
      //if (selectControlTree)
      //  this.controlTree.setSelectedNodes(this.selectedNodes);
      this.editSelectedNodes();
  ...
  selectElement(elem, selectMultiple){
    if (!elem || !elem['UCNode']) return false;
  ...
  onTreeControlSelected(node){
    this.selectNode(node, this.ctrlPressed, false)
  }
  /**
   * 
   ...
   * @param {CNode} node 
   */
  onTreeControlMouseOver(event, node){
    if (this.ctrlPressed && event.buttons == 1)
      this.selectNode(node, this.ctrlPressed, false)
  }
  ...
  selectElementsWithin(parentElement, x1, y1, x2, y2) {
    ...
    controls.each(elem=>{
      ...
      var elX = (elem as any).UCNode.x;
      var elY = (elem as any).UCNode.y;
  ...
  editSelectedNodes(){
    ...
    if (this.selectedNodes.length == 1){
      ...
      this.data.w = node.w;
      this.data.h = node.h;
      this.data.layerN = node.layerN ? parseInt(node.layerN) : null;
  ...
  updateSelectedNodes(){
    if (this.selectedNodes.length == 1){
      ...
      if (node instanceof TabLayer) {
        // Tab doesn't use these values any way
  ...
  async parse(text, pendingSave = false){
  ...
  async renderControls(controllName, nodes, container){
    ...
    var filteredNodes = Objects.filter(nodes, node => node.region==controllName && node.container == container);
    ...
    // @ts-ignore
    //var filterFields = Objects.keyBy(Objects.filter(filteredNodes, n=> (n instanceof FilterfieldtextNode )), 'controllN');

    Objects.forEach(filteredNodes, (anyNode)=>{
      // @ts-ignore
      if (anyNode.__proto__ instanceof ControlNode || anyNode instanceof SetScreenSizeNode) {
        ...
        if (!this.visibleLayers.includes(node.layerN) && !(anyNode instanceof SetScreenSizeNode)) {
          if (node instanceof TabLayer && this.visibleLayers.includes(node.parentN )) {
            //tab visisble
          } else {
            return;
          }
        ...
        if ((node instanceof BackgroundNode || node instanceof LedNode || node instanceof ButtonNode || node instanceof TabLayer)){
          ...
          if ((node instanceof LedNode || node instanceof ButtonNode ) && node.blink) {
  ...
  getControllerItems(){
    const items = Objects.map(this.parser.getRegions(), el=>{
      return {value: el, title:el};
    });
    return (items as any).filter((el: any)=>el.value!="SCREENPROPERTIES");
  }
...
<div class="welcome" [if]="!this.isScreensetLoaded">
  <div class="welcome-box">
    ...
    <div class="mb-4 mt-2">
      UCCNC Screen Editor v{{this.version}} by Eldar Gerfanov
    </div>

    <div [if]="false">
      <div class="mb-1">
        Start by sining in to the app:
      </div>
      <div class="mb-1">
        <button type="button" class="btn btn-xl btn-primary" data-cy="onSignInRegisterClicked" onclick="this.onSignInRegisterClicked()"><i class="fa fa-user"></i> Sign In / Register</button>
      </div>
    </div>
    <div class="mb-3">
      <!-- <div class="mb-1">
      ...
      <div [if]="this.LastSession">
        ...
        <div class="mb-1">
          <button type="button" class="btn btn-xl btn-primary" data-cy="onRestoreSessionClicked" onclick="this.onRestoreSessionClicked()"><i class="fas fa-folder-open"></i> Restore Last Session</button>
        </div>
      </div>
      <div class="mb-1">
        <button type="button" class="btn btn-xl btn-primary" data-cy="onLoadScreensetClicked" onclick="this.onLoadScreensetClicked()"><i class="fas fa-folder-open"></i> Load Screenset from your computer</button>
      </div>
    </div>
    ...
    <div class="mb-1">
      <button type="button" class="btn btn-xl btn-primary" data-cy="onLoadSampleScreensetClicked1" onclick="this.onLoadSampleScreensetClicked(1)">Load Sample Defaultscreenset</button>
    </div>
    <div class="mb-1">
      <button type="button" class="btn btn-xl btn-primary" data-cy="onLoadSampleScreensetClicked2" onclick="this.onLoadSampleScreensetClicked(2)">Load Sample Default2019</button>
    </div>
...
<div class="wrapper" [show]="this.isScreensetLoaded">
  ...
  <div class="right panel tree-container">
    ...
    <div class="panel-body scroll">
      <div [component]="this.controlTree"  [onControlSelected]="this.onTreeControlSelected" [onControlMouseOver]="this.onTreeControlMouseOver"></div>
    </div>
  </div>
  <div class="right">
    <div class="buttons mb-1">
      <button type="button" class="" data-cy="onLoadScreensetClicked1" onclick="this.onLoadScreensetClicked()" title="Load Screenset ..."><i class="fas fa-folder-open"></i></button>
      <button type="button" [if]="this.fileHandle" class="" data-cy="onSaveScreensetClicked" onclick="this.onSaveScreensetClicked()" title="Save Screenset" [disabled]="!this.pendingSave || this.parser.getNodes().length==0"><i class="fas fa-save"></i></button>
      <button type="button" class="" data-cy="onSaveAsScreensetClicked" onclick="this.onSaveAsScreensetClicked()" title="Save Screenset As ..." [disabled]="this.parser.getNodes().length==0">Save As</button>
      <button type="button" [if]="this.data.controller" class="" title="Edit Screen Properties" data-cy="onEditScreenProperttiesClicked" onclick="this.onEditScreenProperttiesClicked()"><i class="fas fa-desktop"></i></button>
      <button type="button" [if]="this.data.controller" class="" title="Edit Picture List" data-cy="onEditPictureListClicked" onclick="this.onEditPictureListClicked()"><i class="fas fa-images"></i></button>
    </div>
    <div class="buttons mb-1">
      <button type="button" class="" [disabled]="this.history.length <= 1" title="Undo" data-cy="onUndoClicked" onclick="this.onUndoClicked()"><i class="fas fa-undo"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 0  " class="" title="Duplicate Selected Elements" data-cy="onDuplicateControlClicked" onclick="this.onDuplicateControlClicked()">2X</button>
      <button type="button" [if]="this.data.controller" class="" title="Create Element" data-cy="onCreateControlClicked" onclick="this.onCreateControlClicked()"><i class="fas fa-plus-square"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 0" class="danger" title="Delete Selected Elements" data-cy="onDeleteControlsClicked" onclick="this.onDeleteControlsClicked()"><i class="fas fa-trash-alt"></i></button>
      <button type="button" [if]="this.isSelectedElementPictureGenSupported" class="" title="Generate Button Picture" data-cy="onGenerateButtonClicked" onclick="this.onGenerateButtonClicked()"><i class="fas fa-file-image"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 0" class="" title="Apply the selected Elements style on other Controllers" data-cy="onApplyNodesStyleOnOtherControllersClicked" onclick="this.onApplyNodesStyleOnOtherControllersClicked()"><i class="fas fa-project-diagram"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 0" class="" title="Copy Selected Elements to other Controllers" data-cy="onCopySelectedControlsClicked" onclick="this.onCopySelectedControlsClicked()"><i class="fas fa-copy"></i></button>
    </div>
    <div class="buttons mb-1">
      
      <button type="button" [if]="this.selectedNodes.length > 2 && this.areSelectedElementsSimilar()" class="" title="Equal Vertical Spacing" data-cy="onFitYClicked" onclick="this.onFitYClicked()"><i class="fas fa-arrows-alt-v"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 2 && this.areSelectedElementsSimilar()" class="" title="Equal Horisontal Spacing" data-cy="onFitXClicked" onclick="this.onFitXClicked()"><i class="fas fa-arrows-alt-h"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 1" class="" title="Align Left" data-cy="onAlignLeftClicked" onclick="this.onAlignLeftClicked()"><i class="fas fa-align-left"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 1" class="" title="Align Justify" data-cy="onAlignJustifyClicked" onclick="this.onAlignJustifyClicked()"><i class="fas fa-align-justify"></i></button>
      <button type="button" [if]="this.selectedNodes.length > 1" class="" title="Align Right" data-cy="onAlignRightClicked" onclick="this.onAlignRightClicked()"><i class="fas fa-align-right"></i></button>
    </div>
    ...
    <div class="help-baloon">
      ...
      <div class="mb-1">
        Contact us: <span class="link" data-cy="onShowEmailClicked" onclick="this.onShowEmailClicked()">show email</span>
      </div>
    </div>
    ...
    <div class="error-baloon" [if]="this.parseErrors.length > 0" >
      ...
      <div [foreach]="this.parseErrors as error">
        <span data-cy="selectNode" onclick="this.selectNode(error.node)" class="error-title clickable">{{error.type}}</span>{{error.message}}
      </div>

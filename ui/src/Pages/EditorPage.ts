...
export class EditorPage extends DialogPage {
  ...
  onInit() {
    this.cn_b_up = (DOM("#canvas_button_up").first() as HTMLCanvasElement).getContext('2d');
    this.cn_b_down = (DOM("#canvas_button_down").first() as HTMLCanvasElement).getContext('2d');
    this.drawControl();
  }
...
<div class="wrapper">
  ...
  <div class="middle">
    <div class="">
      ...
      <!--<div class="">
        <a id="link"></a>
        <button class="btn btn-primary" data-cy="onSaveButtonClicked" onclick="this.onSaveButtonClicked()">Export Files</button>
      </div>-->
    </div>
    <div class="properties-wrapper">
      <div class="presets">
        ...
        <!--<button data-cy="loadFileClicked" onclick="this.loadFileClicked()">Load File</button>-->
      <div [component]="this.form" class="properties"></div>
    </div>
  </div>
</div>
...
function multiLineText(ctx, text, x, y, w, h, fontHeight) {
  ...
  Objects.forEach(lines, (line, i) => {
    ...
    var lineHeight = (textMetrics.actualBoundingBoxAscent || fontHeight) * 1.25;
    ctx.fillText(line, x + w / 2 - Math.min(textMetrics.width, w) / 2, y + (h / 2 + (textMetrics.actualBoundingBoxAscent || fontHeight) / 2 - lineHeight * (lines.length - 1) / 2) + Number(i) * lineHeight, w);
  })

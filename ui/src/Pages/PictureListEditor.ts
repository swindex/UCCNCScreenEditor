...
export class PictureListEditor extends DialogPage {
  ...
  constructor(dirHandle){
    ...
    this.gallery.onPictureUpSelect = async (item, index)=>{
      ...
      var fileItem = await FileHelpers.selectFileFromDirectoryHandle(dirHandle, "Select UP image file") as FileItem
      if (!fileItem) return;

      var fileContents = await FileHelpers.getFileHandleContents(fileItem.handle) as FileContents;
    ...
    this.gallery.onPictureDownSelect = async (item, index)=>{
      ...
      var fileItem = await FileHelpers.selectFileFromDirectoryHandle(dirHandle, "Select DOWN image file") as FileItem
      if (!fileItem) return;

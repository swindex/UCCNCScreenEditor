...
export class Parser {
  ...
  getNodesWhere(region: string, container: ScreenName, nodeType: any){
    ...
    if (nodeType == TabLayer) {
      nodes = nodes.sort((elA: TabLayer, elB: TabLayer)=>{
  ...
  writeOrder(){
    let elementOrder: WriteOrder[] = [
      ...
      {nodeType:TabLayer, screenName: ScreenName.AS3} ,
      {nodeType:Comment5Node} ,//"//Add tabs to jog screen",
      {nodeType:TabLayer, screenName: ScreenName.AS3jog} ,
...
abstract class NodeBase {
  // Abstract instance method - must be implemented by derived classes.
  abstract getCCode(): string;

  abstract validate(): { type: string; message: string; node: NodeBase }[];

  // Static method in the abstract class.
  static parse(cCode: string): NodeBase {
    throw new Error("Override base parse method!");
  }
}


/** Parent abstract class for all nodes without region */
export abstract class CNode extends NodeBase {
...
/** Parent abstract class for all nodes with a region */
export class CRNode extends NodeBase {
  region:string = null;
...
/** Parent class for all on-screen elements */
export class ScreenControlNode extends CRNode {
  container: ScreenName = null;
  layerN: number = null;
}

/** A node for the start of a screen Region code */
...
/** A node for end of the screen Region code  */
...
/** An Text Node we don't care to parse, just read it as-is and then ouput in the same spot*/
...
/** A base for an on-screen Control element */
...
/** A base for an on-screen Control element that also has a width and height */
export class ControlWHNode extends ControlNode {
  w: number | null = null;
  h: number | null = null;
}

/** A base for an on-screen Control element that also has a width */ 
export class ControlWNode extends ControlNode {
  w: number | null = null;
}

/** On screen Tab element. */
export class TabLayer extends ControlWHNode {
  value: string = "";
  font: string = "";
  align: string = "";
  fontSize: number = 24;
  ...
  static parse(cCode: string = ""): TabLayer | null {
    if (!cCode) return null;
    const m = cCode.match(/^(?<container>AS3|AS3jog)\.Addtab\s*\(\s*"(?<value>(?:\\.|[^\\"])*)"\s*,\s*"(?<font>(?:\\.|[^\\"])*)"\s*,\s*"(?<align>(?:\\.|[^\\"])*)"\s*,\s*(?<fontSize>\d*)\s*,\s*(?<color>\d*)\s*,\s*(?<x>\-?[\d\.]*)\s*,\s*(?<y>\-?[\d\.]*)\s*,\s*(?<w>[\d\.]*)\s*,\s*(?<h>[\d\.]*)\s*,\s*(?<picN>\d*)\s*,\s*(?<layerN>\d*)\s*,\s*(?<parentN>\d*)\s*\)\s*;\s*/);
    if (!m) return null;

    const ret = new TabLayer();
    ret.container = ScreenNameFromString(m.groups!.container);
  ...
  getCCode(): string {
    return `${this.container}.Addtab("${str(this.value)}", "${str(this.font)}", "${str(this.align)}", ${num(this.fontSize)}, ${num(this.color)}, ${num(this.x)}, ${num(this.y)}, ${num(this.w)}, ${num(this.h)}, ${this.picN}, ${this.layerN}, ${this.parentN});`;
  }
}
...
*/
/** A helper method that validates combo box items */
...
/** A helper method that adds combo box items */
...
/** A Helper method that clears combo box items */
...
/** A helper method that updates combo box selection */
...
/** A UI Element that displays an input field */
...
/** A UI Element that shows a slider element */
...
/** G-Code view UI Element. It has fixed height of 339 */
...
/** Indicator element that shows aiether up or down picture. Can blink. */
...
/** A helper method that filters and validates a field value */
...
/** A helper method that sets the size of the JOG Panel Tab (in collapsed view) */
...
/** A Helper method that sets Screen Size */
...
/** A helper method for setting the size of a Jog Panel in expanded view */
...
/** A helper method for setting the relative path to the Bitmap directory inside the Flashscreen directory */
...
/** A UI Element that displays background for a Tab */
...
/** A Node that represents settings for the UCCNC Editor */
